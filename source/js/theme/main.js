...
document.addEventListener("DOMContentLoaded", () => document.querySelector('[data-date]').textContent = new Date().getFullYear());
document.addEventListener('click', event => {
  const tag = event.target;
  if(tag.hasAttribute('data-go')){
    event.preventDefault();
    if(tag.closest('.header')){
      document.querySelectorAll('.header--link-active').forEach( item => item.classList.remove('header--link-active'));
